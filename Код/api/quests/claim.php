...
try {
    ...
    $result = bober_claim_user_quest_reward(
        $conn,
        $sessionUserId,
        $data['scope'] ?? 'daily',
        $data['key'] ?? '',
    ...
    if ($error instanceof InvalidArgumentException) {
        ...
        'Награда за этот квест уже получена.',
        'Награду можно получить только после выполнения квеста.',
        'Квест не найден в текущей ротации.',
        'Ротация квестов уже обновилась. Откройте список квестов заново.',
    ], true)) {
        $statusCode = 409;
    }
