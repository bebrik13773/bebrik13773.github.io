...
- основной рабочий домен приложения: `https://bober-api.gt.tc/`
- остальные домены на сервере используются как короткие ссылки и редиректы
- при проверке прода, ссылок и Telegram-анонсов считать основным именно `bober-api.gt.tc`
- наличие других доменных папок на FTP не означает, что туда нужно выкладывать основной код

## Telegram-анонсы

После заметных пользовательских обновлений, которые уже запушены или выложены на прод, нужно публиковать короткий пост в Telegram-канал проекта.

Правило:

- канал: `-1002052101206`
- пост делать от имени бота, если рабочие данные бота есть в текущей сессии
- текст короткий, с эмодзи, с нормальными отступами между блоками, без длинного changelog
- в посте не писать техническую информацию:
  - без commit hash
  - без `message_id`
  - без названий внутренних файлов
  - без слов про smoke-check, FTP, API-роуты, migration и similar internal details
- писать пост как пользовательское обновление, а не как инженерный отчет
- в посте всегда добавлять ссылки:
  - кликер: `https://bober-api.gt.tc/pages/clicker/`
  - `Летающий бобер`: `https://bober-api.gt.tc/games/fly-beaver/`
- если накопилось несколько мелких пользовательских изменений, их нужно объединять в один аккуратный пост
- если подряд прошло `3` пользовательских запроса / коммита без Telegram-анонса, на следующем релевантном завершении нужно опубликовать один сводный пост
- предпочитаемый формат поста:

  ```text
  🦫 Небольшое обновление

  Исправили и улучшили:
  • пункт 1
  • пункт 2
  • пункт 3

  🎮 Играть в кликер
  https://bober-api.gt.tc/pages/clicker/

  🛩 Летающий бобер
  https://bober-api.gt.tc/games/fly-beaver/
  ```

Ограничение:

- токен бота не хранить в репозитории и не записывать в `AGENTS.md`
- если в текущей сессии нет рабочих Telegram-credentials, сначала запросить их у пользователя
- если изменение сугубо внутреннее и не влияет на игроков, отдельный пост не обязателен
